...
public:
    ...
    int getCellNumberCenter(double x, double x0, double hx) {
        return (int) ((getCellFraction(x, x0, hx) + 1.0) +
...
// Simple : C++ (i+i1) ----->>>> F(i+i1-1)
    ...
    double compareParticleLists(Cell *c) {
        ...
        res = compare(doubParticleArray, c->doubParticleArray, number_of_particles * sizeof(Particle) / sizeof(double),
    ...
    double
    checkCellParticles(double *x, double *y, double *z, double *px, double *py, double *pz, double q_m, double m) {
