...
__host__ __device__
#endif
double Cell::get_hx() { return hx; }

#ifdef __CUDACC__
__host__ __device__
#endif
double Cell::get_hy() { return hy; }

#ifdef __CUDACC__
__host__ __device__
#endif
double Cell::get_hz() { return hz; }

#ifdef GPU_PARTICLE

double *Cell::GetParticles() { return doubParticleArray; }
...
__host__ __device__
#endif
int Cell::getCellNumber(double x, double x0, double hx) { return ((int) (getCellFraction(x, x0, hx) + 1.0) + 1); }
...
__host__ __device__
#endif
int Cell::getPointPosition(double x, double x0, double hx) { return (int) getCellFraction(x, x0, hx); }
...
__host__ __device__
#endif
double Cell::getCellReminder(double x, double x0, double hx) {
...
__host__ __device__
#endif
double Cell::getCellCenterReminder(double x, double x0, double hx) {
...
__host__ __device__
#endif
double Cell::getCellTransitRatio(double z1, double z, double z2) { return (z2 - z) / (z1 - z); }


#ifdef __CUDACC__
__host__ __device__
#endif
double Cell::getCellTransitProduct(double z1, double z, double z2) { return (z2 - z) * (z1 - z); }


#ifdef __CUDACC__
__host__ __device__
#endif
double Cell::getRatioBasedX(double x1, double x, double s) { return (x + (x1 - x) * s); }


#ifdef __CUDACC__
__host__ __device__
#endif
double Cell::getCenterRelatedShift(double x, double x1, int i, double hx, double x0) { //double t = 0.50*(x+x1);
...
__host__ __device__
#endif
__host__ __device__ void Cell::inverseDirection(int *dx, int *dy, int *dz) {
    *dx = (*dx == 2) ? 0 : (*dx == 0 ? 2 : 1);
    *dy = (*dy == 2) ? 0 : (*dy == 0 ? 2 : 1);
    *dz = (*dz == 2) ? 0 : (*dz == 0 ? 2 : 1);
}
...
__host__ __device__
#endif
void Cell::ClearParticles() { number_of_particles = 0; }
...
virtual
#endif
void Cell::InverseKernel(double3 x, int3 &i, int3 &i1,
                   double &s1, double &s2, double &s3, double &s4, double &s5, double &s6,
                   double &s11, double &s21, double &s31, double &s41, double &s51, double &s61,
                   Particle *p
) {
...
Field Cell::GetField(Particle *p, CellTotalField cf) {
    ...
    InverseKernel(x, i, i1, s1, s2, s3, s4, s5, s6, s11, s21, s31, s41, s51, s61, p);

    fd.E = GetElectricField(i, i1, s1, s2, s3, s4, s5, s6, s11, s21, s31, s41, s51, s61, p, cf.Ex, cf.Ey, cf.Ez);

    fd.H = GetMagneticField(i, i1, s1, s2, s3, s4, s5, s6, s11, s21, s31, s41, s51, s61, p, cf.Hx, cf.Hy, cf.Hz);
    return fd;
}
...
virtual
#endif
void Cell::Kernel(CellDouble &Jx,
...
virtual
#endif
void Cell::pqr(int3 &i, double3 &x, double3 &x1, double &a1, double tau) {
    double dx, dy, dz, a, dx1, dy1, dz1, su, sv, sw, s1, s2, s3;
...
virtual
#endif
void Cell::MoveSingleParticle(unsigned int i, CellTotalField cf) {
    Particle p;
    Field fd;
...
__host__ __device__
#endif
void Cell::readFieldsFromArrays(double *glob_Ex, double *glob_Ey, double *glob_Ez, double *glob_Hx, double *glob_Hy,
...
double Cell::compareParticleLists(Cell *c) {
    char s[100];
    double res;
    ...
    res = compare(doubParticleArray, c->doubParticleArray, number_of_particles * sizeof(Particle) / sizeof(double), s, ABSOLUTE_TOLERANCE);
