...
void Plasma::GetElectricFieldStartsDirs(
        int3 *start,
        int3 *d1,
        int3 *d2,
        int dir
...
void Plasma::copyCellCurrentsToDevice(CellDouble *d_jx, CellDouble *d_jy, CellDouble *d_jz,
...
void Plasma::SetInitialConditions() {}

void Plasma::ParticleSort() {}
...
//                mass,q_mass };
    ...
    std::cout << "cudaFuncSetCacheConfig returns " << cudaStatus << " " << cudaGetErrorString(cudaStatus)
              << std::endl;
//	   GPU_CurrentsAllCells//<<<dimGrid, dimBlock,16000>>>(d_CellArray);//,0,d_Jx,
    ...
    std::cout << "GPU_CurrentsAllCells returns " << cudaStatus << " " << cudaGetErrorString(cudaStatus)
              << std::endl;
    //mass,q_mass);
    puts("end step");
    cudaDeviceSynchronize();
